...
export const fetchConvocatoriasArbitro = async (id: number, token: string): Promise<ConvocatoriaResponse> => {
  try {

    if (!id || !token) {
      throw new Error('ID and token are required');
    }
    const response = await fetch(`${API_BASE_URL}/api/app-native-api/convocatorias/arbitro/${id}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
      },
